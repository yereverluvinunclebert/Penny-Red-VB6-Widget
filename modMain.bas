...
' Dependencies:
'
'           Requires a Penny Red folder in C:\Users\<user>\AppData\Roaming\ eg: C:\Users\<user>\AppData\Roaming\PennyRed
'           Requires a settings.ini file to exist in C:\Users\<user>\AppData\Roaming\PennyRed
...
'           supplied resource file Penny Red.RES. It is compiled into the binary.
'
'           * OLEGuids.tlb
'
...
'           directly for a copy. The file "install PennyRed 0.1.0.s2g" is the configuration file for setup2go. When you build it will
...
'           RC5's transparency capability is used for the main stamp and the about image only. I haven't used
...
'
'           There is one useful .BAT file - unhide.bat which will reveal a Penny Red stamp mistakenly 'hidden' for an
...
Public Sub mainRoutine(ByVal restart As Boolean)
   ...
   On Error GoTo main_routine_Error
    ...
    Call readSettingsFile("Software\PennyRed", PrSettingsFile)
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If PrFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\PennyRed", "firstTimeRun", PrFirstTimeRun, PrSettingsFile
    End If
...
Private Sub addImagesToImageList()
    ...
    'add Resources to the global ImageList
    Cairo.ImageList.AddImage "pennyRed", App.Path & "\Resources\images\penny red.png"
    Cairo.ImageList.AddImage "pennyBlack", App.Path & "\Resources\images\Penny Black.png"
    Cairo.ImageList.AddImage "twoPennyBlue", App.Path & "\Resources\images\Two Penny Blue.png"
    Cairo.ImageList.AddImage "hongKong10Dollars", App.Path & "\Resources\images\Hong Kong 10 Dollars.png"
    Cairo.ImageList.AddImage "hongKong30Cents", App.Path & "\Resources\images\Hong Kong 30 Cents.png"
...
' Author    : beededea
' Date      : 27/04/2023
' Purpose   : called at runtime and on restart, sets the characteristics of the stamp and menus
'---------------------------------------------------------------------------------------
'
Public Sub adjustMainControls()
   ...
   On Error GoTo adjustMainControls_Error
    ...
    If stampWidget.Hidden = False Then
        stampWidget.opacity = Val(PrOpacity) / 100
        stampWidget.Widget.Refresh
    End If
    ...
    If PrPreventDragging = "0" Then
        ...
        stampWidget.Locked = False
    Else
        menuForm.mnuLockWidget.Checked = True
        stampWidget.Locked = True ' this is just here for continuity's sake, it is also set at the time the control is selected
    End If
    
    If PrShowTaskbar = "0" Then
        fMain.stampForm.ShowInTaskbar = False
    Else
        fMain.stampForm.ShowInTaskbar = True
    End If
...
Public Sub setWindowZordering()
   ...
   On Error GoTo setWindowZordering_Error
    ...
    If Val(PrWindowLevel) = 0 Then
        Call SetWindowPos(fMain.stampForm.hwnd, HWND_BOTTOM, 0&, 0&, 0&, 0&, OnTopFlags)
    ElseIf Val(PrWindowLevel) = 1 Then
        Call SetWindowPos(fMain.stampForm.hwnd, HWND_TOP, 0&, 0&, 0&, 0&, OnTopFlags)
    ElseIf Val(PrWindowLevel) = 2 Then
        Call SetWindowPos(fMain.stampForm.hwnd, HWND_TOPMOST, 0&, 0&, 0&, 0&, OnTopFlags)
    End If
...
'---------------------------------------------------------------------------------------
'
Public Sub readSettingsFile(ByVal location As String, ByVal PrSettingsFile As String)
    On Error GoTo readSettingsFile_Error

    If fFExists(PrSettingsFile) Then
        
        ' general
        PrStartup = fGetINISetting(location, "startup", PrSettingsFile)
        PrGaugeFunctions = fGetINISetting(location, "gaugeFunctions", PrSettingsFile)
        PrStampSelection = fGetINISetting(location, "stampSelection", PrSettingsFile)
        
        ' configuration
        PrEnableTooltips = fGetINISetting(location, "enableTooltips", PrSettingsFile)
        PrEnableBalloonTooltips = fGetINISetting(location, "enableBalloonTooltips", PrSettingsFile)
        PrShowTaskbar = fGetINISetting(location, "showTaskbar", PrSettingsFile)
        
        PrGaugeSize = fGetINISetting(location, "gaugeSize", PrSettingsFile)
        PrScrollWheelDirection = fGetINISetting(location, "scrollWheelDirection", PrSettingsFile)
        'PrWidgetSkew = fGetINISetting(location, "widgetSkew", PrSettingsFile)
        
        ' position
        PrAspectHidden = fGetINISetting(location, "aspectHidden", PrSettingsFile)
        PrWidgetPosition = fGetINISetting(location, "widgetPosition", PrSettingsFile)
        PrWidgetLandscape = fGetINISetting(location, "widgetLandscape", PrSettingsFile)
        PrWidgetPortrait = fGetINISetting(location, "widgetPortrait", PrSettingsFile)
        PrLandscapeFormHoffset = fGetINISetting(location, "landscapeHoffset", PrSettingsFile)
        PrLandscapeFormVoffset = fGetINISetting(location, "landscapeYoffset", PrSettingsFile)
        PrPortraitHoffset = fGetINISetting(location, "portraitHoffset", PrSettingsFile)
        PrPortraitYoffset = fGetINISetting(location, "portraitYoffset", PrSettingsFile)
        PrvLocationPercPrefValue = fGetINISetting(location, "vLocationPercPrefValue", PrSettingsFile)
        PrhLocationPercPrefValue = fGetINISetting(location, "hLocationPercPrefValue", PrSettingsFile)

        ' font
        PrPrefsFont = fGetINISetting(location, "prefsFont", PrSettingsFile)
        PrPrefsFontSize = fGetINISetting(location, "prefsFontSize", PrSettingsFile)
        PrPrefsFontItalics = fGetINISetting(location, "prefsFontItalics", PrSettingsFile)
        PrPrefsFontColour = fGetINISetting(location, "prefsFontColour", PrSettingsFile)
        
        ' sound
        PrEnableSounds = fGetINISetting(location, "enableSounds", PrSettingsFile)
        
        ' development
        PrDebug = fGetINISetting(location, "debug", PrSettingsFile)
        PrDblClickCommand = fGetINISetting(location, "dblClickCommand", PrSettingsFile)
        PrOpenFile = fGetINISetting(location, "openFile", PrSettingsFile)
        PrDefaultEditor = fGetINISetting(location, "defaultEditor", PrSettingsFile)
        
        ' other
        PrMaximiseFormX = fGetINISetting("Software\PennyRed", "maximiseFormX", PrSettingsFile)
        PrMaximiseFormY = fGetINISetting("Software\PennyRed", "maximiseFormY", PrSettingsFile)
        PrLastSelectedTab = fGetINISetting(location, "lastSelectedTab", PrSettingsFile)
        PrSkinTheme = fGetINISetting(location, "skinTheme", PrSettingsFile)
        
        ' window
        PrWindowLevel = fGetINISetting(location, "windowLevel", PrSettingsFile)
        PrPreventDragging = fGetINISetting(location, "preventDragging", PrSettingsFile)
        PrOpacity = fGetINISetting(location, "opacity", PrSettingsFile)
        
        ' we do not want the widget to hide at startup
        'PrWidgetHidden = fGetINISetting(location, "widgetHidden", PrSettingsFile)
        PrWidgetHidden = "0"
        
        PrHidingTime = fGetINISetting(location, "hidingTime", PrSettingsFile)
        PrIgnoreMouse = fGetINISetting(location, "ignoreMouse", PrSettingsFile)
         
        PrFirstTimeRun = fGetINISetting(location, "firstTimeRun", PrSettingsFile)
...
Public Sub validateInputs()
   ...
   On Error GoTo validateInputs_Error
        ...
        If PrStampSelection = vbNullString Then PrStampSelection = "0"
        ...
        ' fonts
        If PrPrefsFont = vbNullString Then PrPrefsFont = "times new roman" 'prefsFont", PrSettingsFile)
        If PrPrefsFontSize = vbNullString Then PrPrefsFontSize = "8" 'prefsFontSize", PrSettingsFile)
        ...
        ' window
        If PrWindowLevel = vbNullString Then PrWindowLevel = "1" 'WindowLevel", PrSettingsFile)
...
Private Sub getToolSettingsFile()
    ...
    PrSettingsDir = fSpecialFolder(feUserAppData) & "\PennyRed" ' just for this user alone
    PrSettingsFile = PrSettingsDir & "\settings.ini"
    ...
    If Not fFExists(PrSettingsFile) Then

        iFileNo = FreeFile
        'open the file for writing
        Open PrSettingsFile For Output As #iFileNo
        Close #iFileNo
    End If
...
Private Sub configureTimers()
    ...
    oldPrSettingsModificationTime = FileDateTime(PrSettingsFile)
...
'
'---------------------------------------------------------------------------------------
' Procedure : setVisibleStampType
' Author    : beededea
' Date      : 07/05/2023
' Purpose   : set the setamp type, can't read the type from comboxbox as the prefs isn't yet open
'---------------------------------------------------------------------------------------
'
Private Sub setVisibleStampType()
    Dim stampName As String: stampName = vbNullString
    On Error GoTo setVisibleStampType_Error

    If PrStampSelection = "0" Then stampName = "Penny Red"
    If PrStampSelection = "1" Then stampName = "Penny Black"
    If PrStampSelection = "2" Then stampName = "Two Penny Blue"
    If PrStampSelection = "3" Then stampName = "Hong Kong 10 Dollars"
    If PrStampSelection = "4" Then stampName = "Hong Kong 30 Cents"
    If PrStampSelection = "5" Then stampName = "Empire Exhibition"
    
'    If cmbStampSelection.ListIndex = 0 Then fullPath = resourcePath & "\penny red" & " white.jpg"
'    If cmbStampSelection.ListIndex = 1 Then fullPath = resourcePath & "\Penny Black" & " white.jpg"
'    If cmbStampSelection.ListIndex = 2 Then fullPath = resourcePath & "\Two Penny Blue" & " white.jpg"
'    If cmbStampSelection.ListIndex = 3 Then fullPath = resourcePath & "\Hong Kong 10 Dollars" & " white.jpg"
'    If cmbStampSelection.ListIndex = 4 Then fullPath = resourcePath & "\Hong Kong 30 Cents" & " white.jpg"
'    If cmbStampSelection.ListIndex = 5 Then fullPath = resourcePath & "\Empire Exhibition" & " white.jpg"

    On Error GoTo 0
    Exit Sub

setVisibleStampType_Error:

    With Err
         If .Number <> 0 Then
            MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure setVisibleStampType of Module modMain"
            Resume Next
          End If
    End With
...
Private Sub createFormOnCurrentDisplay()
    ...
    With New_c.Displays(1) 'get the current Display
      fMain.InitAndShowAsFreeForm .WorkLeft, .WorkTop, 1000, 1000, "Penny Red Widget"
    End With
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        PrUnhide = "true"
        sPutINISetting "Software\PennyRed", "unhide", PrUnhide, PrSettingsFile
        Call stampForm_Unload
        End
    End If
