...
Public PrStampSelection As String
...
Public Sub checkLicenceState()
    ...
    If fFExists(PrSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PennyRed", "Licence", PrSettingsFile)
...
Public Sub setMainTooltips()
   On Error GoTo setMainTooltips_Error
    ...
    If PrEnableTooltips = "1" Then
        'stampWidget.Widget.FontName = PrPrefsFont ' does not apply to the tooltip
        stampWidget.Widget.ToolTip = "Use CTRL+mouse scrollwheel up/down to resize."
    Else
        stampWidget.Widget.ToolTip = ""
    End If
...
Public Sub makeVisibleFormElements()
    ...
    fMain.stampForm.Left = Val(fGetINISetting("Software\PennyRed", "maximiseFormX", PrSettingsFile)) ' / screenPixelsPerPixelX
    fMain.stampForm.Top = Val(fGetINISetting("Software\PennyRed", "maximiseFormY", PrSettingsFile)) ' / screenPixelsPerPixelY

    ' The RC forms are measured in pixels, do remember that...

    fMain.stampForm.show
   ...
   On Error GoTo mainScreen_Error
    ...
    If aspectRatio = "landscape" Then
        If PrWidgetLandscape = "1" Then
            If PrLandscapeFormHoffset <> vbNullString Then
                fMain.stampForm.Left = Val(PrLandscapeFormHoffset)
                fMain.stampForm.Top = Val(PrLandscapeFormVoffset)
            End If
        End If
        If PrAspectHidden = "2" Then
            'Print "Hiding the widget for landscape mode"
            stampWidget.opacity = 0
        End If
    End If
    ...
    If aspectRatio = "portrait" Then
        If PrWidgetPortrait = "1" Then
            fMain.stampForm.Left = Val(PrPortraitHoffset)
            fMain.stampForm.Top = Val(PrPortraitYoffset)
        End If
    ...
    If fMain.stampForm.Left < 0 Then
        fMain.stampForm.Left = 10
    End If
    If fMain.stampForm.Top < 0 Then
        fMain.stampForm.Top = 0
    End If
    If fMain.stampForm.Left > screenWidthPixels - 50 Then
        fMain.stampForm.Left = screenWidthPixels - 150
    End If
    If fMain.stampForm.Top > screenHeightPixels - 50 Then
        fMain.stampForm.Top = screenHeightPixels - 150
    End If

    ' calculate the current hlocation in % of the screen
    ' store the current hlocation in % of the screen
    If PrWidgetPosition = "1" Then
        PrhLocationPercPrefValue = Str$(fMain.stampForm.Left / screenWidthPixels * 100)
        PrvLocationPercPrefValue = Str$(fMain.stampForm.Top / screenHeightPixels * 100)
    End If
...
'---------------------------------------------------------------------------------------
' Procedure : stampForm_Unload
' Author    : beededea
' Date      : 18/08/2022
' Purpose   : the standard form unload routine
'---------------------------------------------------------------------------------------
'
Public Sub stampForm_Unload() ' name follows VB6 standard naming convention
    
    On Error GoTo Form_Unload_Error
    
    PrMaximiseFormX = Str$(fMain.stampForm.Left) ' saving in pixels
    PrMaximiseFormY = Str$(fMain.stampForm.Top)
...
Public Sub makeProgramPreferencesAvailable()
    ...
    If pennyRedPrefs.IsVisible = False Then
    
        If pennyRedPrefs.WindowState = vbMinimized Then
            pennyRedPrefs.WindowState = vbNormal
            'Call readPrefsPosition
        End If
                ' set the current position of the utility according to previously stored positions

        If pennyRedPrefs.WindowState = vbNormal Then
        
            Call readPrefsPosition
            
            If pennyRedPrefs.Left = 0 Then
                If ((fMain.stampForm.Left + fMain.stampForm.Width) * screenTwipsPerPixelX) + 200 + pennyRedPrefs.Width > screenWidthTwips Then
                    pennyRedPrefs.Left = (fMain.stampForm.Left * screenTwipsPerPixelX) - (pennyRedPrefs.Width + 200)
                End If
            End If
            
            If pennyRedPrefs.Left < 0 Then pennyRedPrefs.Left = 0
            If pennyRedPrefs.Top < 0 Then pennyRedPrefs.Top = 0
            
            pennyRedPrefs.show  ' show it again
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    PrFormXPosTwips = fGetINISetting("Software\PennyRed", "formXPos", PrSettingsFile)
    PrFormYPosTwips = fGetINISetting("Software\PennyRed", "formYPos", PrSettingsFile)

    ' if a current location not stored then position to the middle of the screen
    If PrFormXPosTwips <> "" Then
        pennyRedPrefs.Left = Val(PrFormXPosTwips)
    Else
        pennyRedPrefs.Left = screenWidthTwips / 2 - pennyRedPrefs.Width / 2
    End If

    If PrFormYPosTwips <> "" Then
        pennyRedPrefs.Top = Val(PrFormYPosTwips)
    Else
        pennyRedPrefs.Top = Screen.Height / 2 - pennyRedPrefs.Height / 2
    End If
...
Public Sub writePrefsPosition()
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If pennyRedPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        PrFormXPosTwips = LTrim$(Str$(pennyRedPrefs.Left))
        PrFormYPosTwips = LTrim$(Str$(pennyRedPrefs.Top))
        
        ' now write those params to the toolSettings.ini
        sPutINISetting "Software\PennyRed", "formXPos", PrFormXPosTwips, PrSettingsFile
        sPutINISetting "Software\PennyRed", "formYPos", PrFormYPosTwips, PrSettingsFile
    End If
...
writePrefsPosition_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure writePrefsPosition of Form pennyRedPrefs"
End Sub
...
Private Sub settingsTimer_Timer()
    ...
    If Not fFExists(PrSettingsFile) Then
        MsgBox ("%Err-I-ErrorNumber 13 - FCW was unable to access the dock settings ini file. " & vbCrLf & PrSettingsFile)
        Exit Sub
    End If
    
    ' check the settings.ini file date/time
    settingsModificationTime = FileDateTime(PrSettingsFile)
